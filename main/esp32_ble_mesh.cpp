...
static void ble_indicate(uint8_t value, uint16_t id) {
    if (gl_profile_tab[0].gatts_if == ESP_GATT_IF_NONE) {
        ESP_LOGE(GATTS_TAG, "Gatts_if is none\n");
        return;
    }
    ESP_LOGI(GATTS_TAG, "Notifying client %d\n", id);
...
void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param, uint16_t id_client){
    ...
    if (param->write.need_rsp){
        if (param->write.is_prep){
            ...
            esp_err_t response_err = esp_ble_gatts_send_response(gatts_if, id_client, param->write.trans_id, status, gatt_rsp);
            ...
            esp_ble_gatts_send_response(gatts_if, id_client, param->write.trans_id, status, NULL);
        }
    }
}
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        example_write_event_env(gatts_if, &a_prepare_write_env, param, param->write.conn_id);
        break;
    }
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        // Testing notification process to clients connecting
        /*
        uint8_t * value = malloc(4*sizeof(uint8_t));
        ...
        ESP_LOGI(GATTS_TAG, "Starting to notify the client %d ", param->connect.conn_id);
        int i = 0;
        //for(; i<3; i++) {
		esp_err_t ret = esp_ble_gatts_send_indicate(gatts_if, param->connect.conn_id, param->add_char.attr_handle, sizeof(value), value, false); //param->conf.handle
		if (ret != ESP_OK) ESP_LOGE(GATTS_TAG, "Failure when notifying the client");
			//vTaskDelay(1000);
		//}
        ESP_LOGI(GATTS_TAG, "End of notifications for client %d ", param->connect.conn_id);
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_WRITE_EVT: {
        ...
        example_write_event_env(gatts_if, &b_prepare_write_env, param, param->write.conn_id);
        break;
    }
    case ESP_GATTS_EXEC_WRITE_EVT:{
        ESP_LOGI(GATTS_TAG,"ESP_GATTS_EXEC_WRITE_EVT");
